...
New Project Idea
    ↓
/generate-charter → Define business vision and objectives
    ↓
/generate-prd → Detail product requirements and user stories
    ↓
/generate-architecture → Design system architecture
    ↓
/generate-wbs → Break down work into tasks
    ↓
/generate-task-spec → Create detailed specifications
    ↓
...
Existing Project
    ...
    │   ├─> /generate-task-spec to specify the feature
    │   ├─> Implement using specification
    │   └─> /update-wbs to track completion
    │
    ├─> Major Change/Refactor
    │   ├─> Ensure PRD reflects changes needed
    │   ├─> /update-architecture if architecture changes
    │   ├─> /update-wbs to revise tasks
    │   └─> Implement changes
    │
    └─> Bug Fix/Maintenance
...
Project Milestone
    ├─> Sprint End
    │   ├─> /update-wbs to sync tasks with reality
    │   └─> /update-prd if scope changed
    │
    ├─> Major Feature Complete
    │   ├─> /update-architecture if architecture evolved
    │   └─> Update relevant CLAUDE.md files
    │
    └─> Project Complete
        └─> /update-charter for business outcomes
```
...
```bash
/generate-charter "Building a customer support automation platform for small businesses"
```
...
```bash
/generate-prd "Based on the project charter, create detailed product requirements"
```
...
```bash
/generate-architecture "Design system architecture for the requirements in the PRD"
```
...
```bash
/generate-wbs "Create work breakdown structure from the architecture design"
```
...
/generate-task-spec "Create specification for user authentication feature"
# Implement using the specification
/update-wbs "Update based on last 10 commits"
```
...
```bash
/generate-task-spec "Add real-time notifications feature to existing chat system"
```
...
/update-wbs "Update based on notification feature implementation"
```
...
```bash
# If scope expanded:
/update-prd "Update requirements based on notification feature additions"
```
...
```bash
/update-architecture "Analyze and document architecture changes"
```
...
```bash
# At sprint end:
/update-wbs "Update based on last 2 weeks of work"

# At major milestone:
/update-architecture "Document architecture evolution"

# At project completion:
/update-charter "Update with business outcomes and lessons learned"
```
...
/generate-task-spec "Create detailed spec for [specific feature]"
```
...
/update-prd "Update based on new customer feedback about search functionality"

# Revise tasks:
/update-wbs "Adjust tasks based on PRD changes"

# Track the impact:
/update-architecture "Document any architectural impacts from requirement changes"
```
...
/generate-task-spec "Create detailed specification for user authentication using OAuth2 with Google provider only, no email/password option"
...
- `/generate-charter [business idea]` - Creates business vision and objectives
- `/generate-prd [based on charter]` - Creates detailed product requirements
- `/generate-architecture [based on PRD]` - Creates system architecture
- `/generate-wbs [based on architecture]` - Creates task breakdown

### Development Commands

These commands support active development:

- `/generate-task-spec [feature description]` - Creates detailed implementation specification
- `/generate-prompt [use case]` - Creates reusable LLM prompts
- `/generate-unit-tests [specification]` - Generates focused unit tests

### Update Commands

These commands maintain documentation accuracy through intelligent analysis:

- `/update-wbs [last N commits or days]` - Updates task status and complexity
- `/update-prd [reason for update]` - Updates requirements based on discoveries
- `/update-architecture [after implementation]` - Updates architecture documentation
- `/update-charter [at completion]` - Updates business outcomes
...
Run `/update-wbs` every few days during active development. This catches drift early when it's easy to address. Waiting until sprint end makes updates harder and less accurate.
...
3. Am I implementing a specific feature? Use `/generate-task-spec` for clear specifications.
