...
(function () {
  ...
  function sendToBackground(text, replyMode, fullPageData) {
    return new Promise((resolve, reject) => {
      ...
      console.log('[ChatRipper] Sending to background:', msg.type, msg.platform, msg.replyMode, fullPageData ? '(full page)' : '(selection)');
  ...
  function doAnalyzeRequest(replyMode) {
    ...
    if (platform === 'revio') {
      scrapeRevioAsync().then(function(scraped) {
        console.log('[ChatRipper] Revio analyze scraped:', scraped.type, 'messages:', scraped.messageCount);
  ...
  function requestAnalyzeChat() {
    ...
    const scraped = scrapePageContent();
    console.log('[ChatRipper] Analyze & Chat scraped:', scraped.type, 'messages:', scraped.messageCount);
  ...
  function requestAnalyzeScore() {
    ...
    const scraped = scrapePageContent();
    console.log('[ChatRipper] Analyze & Score scraped:', scraped.type, 'messages:', scraped.messageCount);
